...
<jq:plugin name="DOMWindow"/>
	...
	<div style="float:left; margin-top:8px;"><input type="checkbox" class="checkall"> <g:message code="userList.checkAll"/></div>

    <div style="float:right;" class="controlBarUpload" id="controlBarUpload" >
        <g:link style="font-size: 12px; margin-bottom: 5px; " class="btn thickbox" name="Upload custom list" action="upload"
                params="[keepThis:'true',TB_iframe:'true',height:'350',width:'600',title:'someTitle']"><g:message code="Upload"/></g:link>
    </div>

    <div style="float:right;" class="controlBarUpload" id="controlBarDelete">
        <g:submitButton style="font-size: 12px; margin-bottom: 5px;height: 30px;" class="btn" name="del" value="${message(code: 'userList.deleteLists')}" onclick="return confirm('${message(code: 'userList.confirm')}');" />&nbsp;
    </div>

    <br/>
	
	<table class="listTable table" width="100%" cellpadding="5">
        <thead>
            <th style="width:10%" class="Customer group-word">Tags</th>
            <th style="width:25%" class="group-word">Study</th>
            <th style="width:65% " class="group-false">Lists</th>
        </thead>
        <tbody>
			...
			<td style="background-color:white;">
                    ...
                    <div id="${userListInstance.id}_div" style="collapse:true;border:0px solid red;margin-bottom:5px;padding:3px 3px 3px 3px">
                        ...
                        <g:if test="${session.userId.equals(userListInstance.author.username)}">
                            <div style="border:0px solid black;width:2%;float:left;padding-right:10px"><g:checkBox class="the_checkbox" name="deleteList" value="${userListInstance.id}" checked="false"/></div>
                        </g:if>
                        ...
                        <div id="${userListInstance.id}_title" style="border:0px solid black;height: 30px;">
                            ...
                            <g:if test="${userListInstance.listItems.size() <= 100}">
                                <div id="${userListInstance.id}_toggleContainer" style="border:0px solid black;vertical-align: top">
                                    ...
                                    <div id="${userListInstance.id}_tog" style="float:right;border:0px solid black;height:20px; cursor: pointer;" onclick="toggle('${userListInstance.id}');">
                                        <img class="${userListInstance.id}_toggle"src="${createLinkTo(dir: 'images', file: 'expand.gif')}" width="13" height="14" border="0" alt="Show/Hide" title="Show/Hide" onclick="if($('#${userListInstance.id}_listItems').length == 0){var classn = this.className; {${remoteFunction(action:'getListItems',id:userListInstance.id,onLoading:'showPageSpinner(true,classn)',onComplete:'showPageSpinner(false,classn)',update:userListInstance.id+'_content')}return false;}}"/>
                                        <img class="${userListInstance.id}_toggle" src="${createLinkTo(dir: 'images', file: 'collapse.gif')}" width="13" height="14" border="0" alt="Show/Hide" title="Show/Hide" style="display:none" />
                        ...
                        <g:if test="${session.userId.equals(userListInstance.author.username)}">
                            ...
                            <div style="border:0px solid black;width:100%;float:right; text-align: right; background-color:#f3f3f3;padding-bottom:5px;">
                                <g:link action="listModify" params="[id:userListInstance.id,name:userListInstance.name,mode:'View']" style="padding-right:5px" class="editWindow"><img alt="edit analysis" title="View/Modify List" src="${createLinkTo(dir: 'images', file: 'zoom_in.png')}"  border="0" /></g:link>
                                <g:if test="${!userListInstance.tags.contains('_temporary')}">
                                    <g:link class="thickbox" name="Share &nbsp; ${userListInstance.name}" action="share" controller="share" params="[id:userListInstance.id,name:userListInstance.name,type:'USER_LIST',keepThis:'true',TB_iframe:'true',height:'375',width:'450',title:'someTitle']"><img alt="share list or view groups previously shared to" title="Share list" style="height: 18px;padding-right:5px" src="${createLinkTo(dir: 'images', file: 'share.png')}" border="0"/></a></g:link>
                                </g:if>
                        ...
                        <div id="${userListInstance.id}_content" style="border:0px solid black;display:none; background-color:#f3f3f3;padding-bottom:10px;float: right;">
                        </div>

                        <div style="width:450px;border-bottom:0px solid grey;background-color:#f3f3f3;padding:5px 5px 5px 10px;"float: right;">
